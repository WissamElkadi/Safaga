...
targetArch='all'
buildType='all'
outputDirectory="safaga_build"

declare -a targetArchArray=()
declare -a buildTypeArray=()

usage="${PURPLE} $(basename "$0") [-h] [-t a b o] -- program to build and install Safaga :${WHITE}
    ...
    - o  ${GREEN} [Optional] ${CYAN} output build directory default {safaga_build} ${WHITE}"
...
if [[ $taregtOS = ' ' ]]; then
    ...
    if [ $taregtOS = 'android' ]; then
        ...
        if [ $taregtOS = 'windows' ]; then
            ...
            buildCommand="MSBuild.exe Safaga.sln"
        fi
    fi
fi
...
for bt in "${buildTypeArray[@]}" ; do
    ...
    for ta in "${targetArchArray[@]}" ; do
        ...
        cmake -G "$generator" -DCMAKE_BUILD_TYPE=$bt -DCMAKE_TOOLCHAIN_FILE="../../../../Safaga/build/platforms/$taregtOS/$taregtOS.toolchain.$ta.cmake" ../../../../Safaga/build
        $buildCommand
