...
namespace Safaga
{
	...
	{
		template<typename T>
		Uniform<T>::Uniform(std::string _name, MemoryFormat _MemoryFormat)
		{
			mName = _name;
		...
		template<typename T>
		Uniform<T>::Uniform(std::string _name, MemoryFormat _MemoryFormat, T* _value)
		{
			mName = _name;
			mMemoryFormat = _MemoryFormat;
			mValue = _value;
		...
		template<typename T>
		void Uniform<T>::updateValue(T* _value)
		{
			mValue = _value;
		}

		template<typename T>
		void Uniform<T>::updateValue(int _programId, const T* _value)
		{
		...
		template<typename T>
		MemoryFormat Uniform<T>::getMemoryFormat()
		{
			return mMemoryFormat;
		}

		template<typename T>
		void Uniform<T>::update(int _programId)
		{
