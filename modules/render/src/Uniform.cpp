...
namespace Platform
{
	...
	{
		template<typename T>
		Uniform::Uniform(std::string _name, MemoryFormat _MemoryFormat)
		{
			mName = _name;
		...
		template<typename T>
		Uniform::Uniform(std::string _name, MemoryFormat _MemoryFormat, T* _value)
		{
			mName = _name;
			mMemoryFormat = _MemoryFormat;
			mValue = _value;
		...
		template<typename T>
		void Uniform::updateValue(T* _value)
		{
			mValue = _value;
		}

		template<typename T>
		void Uniform::updateValue(int _programId, const T* _value)
		{
		...
		template<typename T>
		MemoryFormat Uniform::getMemoryFormat() override
		{
			return mMemoryFormat;
		}

		template<typename T>
		void Uniform::update(int _programId)
		{
